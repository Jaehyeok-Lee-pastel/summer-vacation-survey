...
class VacationRecommendationService:
    ...
    def _learn_patterns(self):
        ...
        for _, row in satisfied_data.iterrows():
            ...
            # 🔧 수정: '기타' 값들을 필터링
            if vacation_type == '기타' or location_type == '기타':
                continue  # '기타' 데이터는 학습에서 제외
                
            location = (row.get('휴가_장소') or 
                    row.get('domestic_location') or     
                    row.get('overseas_location'))
            
            # 🔧 location이 비어있거나 '기타'이거나 location_type과 같으면 스킵
            if (not location or 
    ...
    def _generate_recommendations(self, user_data, similar_users):
        """사용자 설문에 기반한 5개 추천 생성"""
        recommendations = []
        
        # 1단계: 사용자가 설문에서 선택한 값 추출
        user_vacation_type = user_data.get('가장_최근_여름_휴가')  # 예: '문화생활'
        user_location_type = user_data.get('휴가_장소_국내_해외')   # 예: '국내'
        
        print(f"사용자 선택: {user_vacation_type} + {user_location_type}")
        
        # 2단계: 사용자가 선택한 조합을 최우선 추천
        if (user_vacation_type and user_vacation_type in self.vacation_patterns and
            user_location_type and user_location_type in self.vacation_patterns[user_vacation_type]):
            
            experiences = self.vacation_patterns[user_vacation_type][user_location_type]
            ...
            if len(experiences) >= 2:
                ...
                if avg_satisfaction >= 3.0:
                    ...
                    print(f"사용자 맞춤 추천: {top_location[0]}")
                    
                    recommendations.append({
                        'vacation_type': user_vacation_type,
                        'location_type': user_location_type,
                        'recommended_location': top_location[0],
                        'avg_satisfaction': round(avg_satisfaction, 2),
                        'experience_count': len(experiences),
                        'confidence': self._calculate_confidence(experiences, avg_satisfaction)
                    })
        
        # 3단계: 나머지 4개를 다른 조합에서 선택 (만족도 높은 순)
        other_recommendations = []
        
        for vacation_type, location_data in self.vacation_patterns.items():
            for location_type, experiences in location_data.items():
                # 사용자가 이미 선택한 조합은 제외
                if (vacation_type == user_vacation_type and location_type == user_location_type):
                    continue
        ...
        # 4단계: 만족도와 경험 수 기준으로 정렬하여 상위 4개 선택
        other_recommendations.sort(key=lambda x: (x['avg_satisfaction'], x['experience_count']), reverse=True)
        
        # 5단계: 사용자 맞춤 추천(1개) + 기타 추천(4개) = 총 5개
        recommendations.extend(other_recommendations[:4])
        
        # 만약 사용자 맞춤 추천이 없으면 상위 5개 선택
        if len(recommendations) == 0:
            recommendations = other_recommendations[:5]
        elif len(recommendations) < 5:
            # 부족한 경우 더 추가
            needed = 5 - len(recommendations)
            recommendations.extend(other_recommendations[4:4+needed])
        
        print(f"최종 추천 {len(recommendations)}개 생성")
        return recommendations[:5]  # 확실히 5개만 반환
    
    def _calculate_confidence(self, experiences, avg_satisfaction):
        """
        현실적인 신뢰도 계산
        - 경험 수: 많을수록 신뢰도 증가 (하지만 급격하지 않게)
        - 만족도: 높을수록 신뢰도 증가
        """
        experience_count = len(experiences)
        
        # 경험 수 기반 점수 (0.3 ~ 0.8 범위)
        # 2개: 0.3, 10개: 0.5, 50개: 0.7, 100개+: 0.8
        if experience_count < 5:
            experience_score = 0.3 + (experience_count - 2) * 0.05
        elif experience_count < 20:
            experience_score = 0.45 + (experience_count - 5) * 0.01
        elif experience_count < 50:
            experience_score = 0.6 + (experience_count - 20) * 0.005
        else:
            experience_score = min(0.75, 0.75 + (experience_count - 50) * 0.0001)
        # 만족도 기반 점수 (0.6 ~ 1.0 범위)
        # 3.0(보통): 0.6, 4.0(만족): 0.8, 5.0(매우만족): 1.0
        satisfaction_score = 0.4 + (avg_satisfaction - 3.0) * 0.3
        
        # 기본 신뢰도 계산
        base_confidence = (experience_score * 0.6) + (satisfaction_score * 0.4)
        
        # 최종 신뢰도는 0.3 ~ 0.95 범위로 제한
